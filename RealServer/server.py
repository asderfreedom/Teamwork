...
import RealServer.FTPserver as fftp
...
class HandlerConnect(threading.Thread):
    def __init__(self, name, socket):
        ...
        self.user = None
        self.logged = False
        # self.friends = []  # 用来记录登陆用户的好友，以及好友在线状态

    def run(self):
        print("{} has soccused connected!")
        while True:
            recv_msg = (self.socket.recv(1024)).decode('utf-8')  # 接收到的信息
            msg = ast.literal_eval(recv_msg)   # 把字符串类型转化为字典类型
            recv_type = msg['msgtype']  # recv_type 是用来分辨接收到的信息类型
            if not self.logged:
                ...
                if recv_type == 0:
                    ...
                    if self.register_manage(msg['email']):
                        ...
                        while not flag:
                            ...
                            flag = db1.adduser(u1)
                        os.makedirs('E:/Desktop/spyder/Projects/Server/data/%s'%(str(u1.No)))
                        self.socket.send(("你的id为%s" % (u1.No)).encode('utf-8'))
                elif recv_type == 1:
    ...
    def img_manage(self):
        # 应用主功能，用户上传图片，处理完成后再返还给用户
        # 初步设想，图片处理完成后，就给用户发送信息，用户再自行下载
        m1 = (self.socket.recv(1024)).decode('utf-8')
        self.socket.send("finish".encode('utf-8'))
        self.socket.send(m1.encode('utf-8'))
